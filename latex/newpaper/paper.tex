...
PrimeKG \citep{primekg2023} exemplifies this usefulness in the biomedical setting. It is a precision medicine knowledge graph containing approximately 129{,}000 nodes and 4 million relations, integrating 20 high-quality biomedical resources to describe 17{,}080 diseases across ten major biological scales. Nodes span ten categories including gene/protein, disease, drug, phenotype, and pathway, while edges span 29 categories including indication, contraindication, off-label use, side effect, drug-target interaction, and pathway membership. The inclusion of indication, contraindication, and off-label use edges -- frequently absent from other biomedical knowledge graphs -- together with paired clinical-guideline text descriptions for drugs and diseases, makes PrimeKG distinctively well suited for multi-modal analyses of disease mechanisms and therapeutic action.

PrimeKG has rapidly become a substrate for downstream precision medicine research. It has been paired with deep graph neural networks to discover disease biomarkers, characterize disease processes, predict biological mechanisms, and identify drug-repurposing candidates; in a recent retrospective analysis, PrimeKG-based scoring placed indicated drugs significantly closer to their target diseases than expected by chance for 8 of 10 disease categories \citep{primekg2023}. Subsequent extensions have integrated patient-level data for personalized drug repurposing \citep{pdr2025} and incorporated complete OMIM rare-disease entries to improve coverage. Methods built on PrimeKG therefore directly inform clinically relevant decisions, which raises the stakes of the modeling choices we make on top of it.

This diversity in node and edge types, while informative, is precisely what makes graph learning on PrimeKG challenging. Standard graph neural networks were designed for homogeneous graphs, and extending them to handle multiple node and edge types introduces substantial architectural and computational overhead. Heterogeneous GNNs must maintain separate parameters per relation type, making training expensive, which raises the question: is the structural overhead worth it?

In many recent systems, particularly retrieval-augmented generation pipelines, the answer has simply been no. These systems pass node text descriptions directly into a language model and ignore the graph entirely, making the assumption that the text content is sufficient. For knowledge graphs with rich node-level text, this assumption is not unreasonable, as two diseases that share similar clinical language are likely to share biological mechanisms, and this may be fully captured by text embeddings alone. If structure adds little beyond what text already encodes, then skipping the graph is computationally convenient. If the structure carries residual predictive signal through patterns of connectivity that the text does not explain, then ignoring it leaves information on the table. This paper investigates which of these regimes holds in PrimeKG, and under what biomedical contexts.
...
For each of the four schemas defined in Table~\ref{tab:schemas} and the four structural embedders introduced in Section~\ref{sec:method}, we learn structural representations on the corresponding schema subgraph and apply the cross-fit DML estimator described above to obtain out-of-sample predictions for every candidate edge. We then quantify the residual contribution of structure by both the average treatment effect $\bar\tau$ and the AUC gap $\Delta\mathrm{AUC}$, and assess significance via a permutation null on $\Delta\mathrm{AUC}$.

\subsection{In-Distribution Estimates}
\label{sec:in-dist}

Table~\ref{tab:in-dist} reports the text-only AUC, the text-plus-structure AUC, the AUC gap, and the corresponding permutation $p$-value for each (schema, embedder) pair. All 16 combinations attain the minimum achievable significance level under $B=50$ permutations ($p_{\mathrm{AUC}} = 0.020$), indicating that structural features carry information beyond text under every schema--embedder configuration we evaluate.
...
Two patterns emerge from these estimates. First, the magnitude of the AUC gap is inversely related to the strength of the text baseline: in Schemas A and B, where the text-only model already attains $\mathrm{AUC} \geq 0.977$, structure contributes only $+0.01$--$0.02$ AUC; in the comparatively harder Schemas C and D, where the text baseline falls to $0.943$ and $0.917$ respectively, structure contributes $+0.03$--$0.07$ AUC. Figure~\ref{fig:in-dist-bars} visualizes this relationship across all (schema, embedder) pairs. Second, within each schema the three random-walk embedders yield AUC gaps that differ by at most $0.001$, a difference that lies well within the cluster-bootstrap uncertainty for $\bar\tau$ and is not statistically distinguishable. GraphSAGE, by contrast, consistently produces an AUC gap roughly half the magnitude of the walk-based embedders, suggesting that under in-distribution evaluation its parametric neighborhood aggregation captures a strict subset of the residual signal accessible to walk-based co-occurrence statistics.

\subsection{Schema-Transfer Estimates}
\label{sec:transfer}

To assess whether the residual structural signal identified above reflects schema-invariant biological primitives or schema-specific patterns, we conduct a leave-one-schema-out transfer experiment. For each held-out schema, we fit the DML nuisance models on the union of edge pairs from the remaining three schemas and evaluate the resulting estimator on the held-out schema using structural embeddings learned on its own subgraph. Table~\ref{tab:transfer} reports out-of-distribution AUC for the text-only baseline and for each text-plus-structure model, and Figure~\ref{fig:transfer} provides the corresponding visualization.
...
Three findings stand out. First, walk-based embedders transfer universally: all twelve (held-out schema, walk-based embedder) entries satisfy $\mathrm{AUC}_{TS} \geq \mathrm{AUC}_T$, with cluster-bootstrap confidence intervals on the transferred $\bar\tau$ excluding zero in every case. Second, GraphSAGE degrades performance relative to the text baseline in three of the four held-out schemas, most severely on Schema D, where AUC declines from $0.954$ (text only) to $0.768$ (text plus transferred GraphSAGE). Together these results indicate that the residual signal captured by walk-based embeddings is largely schema-invariant, while the residual signal captured by parametric neighborhood aggregation is schema-specific and does not survive substantial distribution shift.

Third, the most informative case is Schema C, on which text trained over the drug-centric source schemas (A, B, D) attains $\mathrm{AUC} = 0.474$ -- effectively at chance -- whereas the addition of random-walk structural features recovers $\mathrm{AUC} \approx 0.93$. This represents a $+0.45$ AUC recovery from a model whose textual nuisance has no predictive purchase on the disease--phenotype--gene/protein context, illustrating that structural embeddings can carry information complementary to, rather than merely correlated with, the textual signal. Finally, although the three random-walk embedders are statistically indistinguishable in-distribution, node2vec-DFS is the strongest transferer for the drug-centric schemas B and D; the BFS--DFS distinction therefore emerges only under distribution shift, suggesting that community-membership and role-based structural primitives differ in their cross-schema invariance even when they encode equivalent within-schema information.

\section{Discussion}

\paragraph{Text-baseline strength governs the apparent value of structure.}
Across four PrimeKG schemas and four structural embedders, graph structure adds significant residual predictive value beyond a Word2Vec text baseline in every (schema, embedder) combination, yet the magnitude of that value is inversely related to text-baseline strength. In schemas where the text-only model already attains near-ceiling AUC, structure contributes only marginal residual signal; in schemas where text is comparatively uninformative, structural embeddings supply most of the remaining predictive content. This pattern aligns with prior observations that GNNs offer limited benefit on graphs whose node features are already highly informative \citep{huang2023}, and it complicates a literal reading of the GraphRAG premise that combining structure and text is uniformly beneficial \citep{grag2024,mor2025}. A more accurate framing, consistent with our estimates, is that the residual value of structure is a function of the text encoder: improvements in textual representation can be expected to compress, though not eliminate, the structural contribution. Recent work on multimodal DML has likewise shown that the choice and depth of the text representation can shift apparent treatment effects substantially \citep{doublemldeep2024,doublelingo2024}, reinforcing the importance of reporting text-baseline strength alongside any structural-value claim.

\paragraph{Within-schema redundancy of walk-based probes.}
The three random-walk embedders we evaluate (DeepWalk, node2vec-BFS, node2vec-DFS) were chosen as probes for distinct structural primitives -- proximity, role, and community membership respectively -- yet within each schema they yield AUC gaps that are statistically indistinguishable. This redundancy suggests that, at least for link prediction on PrimeKG, the residual structural signal is concentrated in low-frequency co-occurrence statistics that all three samplers recover. It is consistent with theoretical analyses showing that DeepWalk and node2vec implicitly factorize closely related variants of the pointwise mutual information matrix of the random-walk transition operator. In our setting, the BFS--DFS distinction emerges only under schema shift, which we interpret as evidence that the discriminating power of these probes lies not in what they encode within a single subgraph but in how the encoded structure transports across biomedical contexts.

\paragraph{Walk-based representations transport; aggregation does not.}
Under leave-one-schema-out transfer, all twelve walk-based (held-out schema, embedder) combinations preserve or improve on the text baseline, while GraphSAGE degrades AUC in three of four held-out schemas. We attribute this asymmetry to the inductive biases of the two families. Walk-based embeddings encode structure at the level of node co-occurrence in random trajectories, a statistic that depends on local connectivity but is largely invariant to the specific node and relation types present in the schema. GraphSAGE's parametric mean aggregator, by contrast, learns transformations that are tied to the feature distribution and neighborhood composition of the training schema, and these transformations need not transport when the held-out schema introduces new node types or markedly different degree distributions. The Schema-C transfer result -- a $+0.45$ AUC recovery from random-baseline text to $\mathrm{AUC}\approx 0.93$ -- is the cleanest demonstration of this asymmetry: random walks supply nearly all of the predictive content for a clinical-presentation context that the source-schema text encoder has no leverage on. This complements the message of recent work on biomedical KG completion such as BioPathNet \citep{biopathnet2025}, which argues that path-based reasoning encodes biologically meaningful regularities, and of analyses showing that semantic and structural features carry largely complementary information in heterogeneous KGs \citep{rpiss2024}.

\paragraph{Beyond PrimeKG: a general diagnostic for structural value.}
Although we instantiate our analysis on PrimeKG, neither the estimand nor the estimator is specific to this graph. The cross-fit DML procedure requires only (i) a binary or continuous edge label, (ii) a text-derived feature for each node or edge, and (iii) one or more candidate structural embeddings; it returns a calibration-invariant magnitude metric ($\Delta\mathrm{AUC}$) and a Neyman-orthogonal point estimate ($\bar\tau$) for the residual contribution of structure beyond text. This makes the framework directly applicable as a pre-modeling diagnostic for any text-attributed knowledge graph -- for example Hetionet, BioKG, the OGB knowledge-graph benchmarks, citation graphs, product graphs, or enterprise knowledge graphs of the kind discussed in \citet{hogan2021}. A practitioner considering whether to invest in heterogeneous-GNN infrastructure for a new graph could first run the procedure on a representative subset, observe whether $\Delta\mathrm{AUC}$ is large enough to justify the engineering and compute cost, and additionally test whether the structural value transports across natural sub-schemas of their graph. We view this kind of structural-value triage as a promising direction for future work: a small library of standard probes (walk-based, aggregation-based, spectral) evaluated against a contemporary text encoder via DML could provide a portable, interpretable answer to the question of whether and how much graph structure is worth using in a given application.
...
\bibitem[BioPathNet(2025)]{biopathnet2025} BioPathNet authors. \newblock Enhancing link prediction in biomedical knowledge graphs with BioPathNet. \newblock \emph{Nature Biomedical Engineering}, 2025.
\bibitem[RP-ISS(2024)]{rpiss2024} RP-ISS authors. \newblock A novel model for relation prediction in knowledge graphs exploiting semantic and structural feature integration. \newblock \emph{Scientific Reports}, 2024.
\bibitem[DoubleMLDeep(2024)]{doublemldeep2024} Klyuchnikov, N., et al. \newblock DoubleMLDeep: Estimation of causal effects with multimodal data. \newblock arXiv:2402.01785, 2024.
\bibitem[DoubleLingo(2024)]{doublelingo2024} Zhang, Y., et al. \newblock DoubleLingo: LLM-based nuisance models for double machine learning. \newblock 2024.
\bibitem[PDR(2025)]{pdr2025} Precision Drug Repurposing authors. \newblock Precision drug repurposing: Patient-level modeling combining a foundational knowledge graph with biobank data. \newblock \emph{Journal of Biomedical Informatics}, 2025.
\end{thebibliography}
