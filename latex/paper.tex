...
\begin{abstract}
We investigate how retrieval strategy affects answer quality in graph retrieval-augmented generation (GraphRAG) as query complexity increases. Through a controlled experiment on the STaRK-PrimeKG benchmark (129K nodes, 4M edges, 11,204 queries), we compare six retrieval strategies---node-centric, path-centric, subgraph-centric, hybrid, beam search, and an adaptive router---stratified by the number of relational hops required to reach the answer. We find a sharp \textit{performance cliff} from 1-hop to 2-hop queries across all strategies, rather than the gradual phase transition we hypothesized. Hit@10 drops approximately 70\% at this boundary. Logistic regression confirms a significant strategy$\times$hop-count interaction ($\beta = +2.09$), indicating structural methods gain relative advantage as complexity increases. Ablation studies reveal a fundamental focus-versus-coverage tradeoff: configurations that maximize 1-hop performance (e.g., single-seed subgraph expansion achieving Hit@10 $= 0.77$) completely fail at multi-hop, while broader configurations that help multi-hop dilute 1-hop accuracy. We further show that two approaches designed to overcome the cliff---embedding-guided beam search and query-adaptive routing via score skewness---fail to do so, confirming that the cliff is a fundamental property of embedding-based retrieval on dense graphs rather than an artifact of naive implementations. We argue that overcoming this barrier requires relation-type-aware traversal, not simply better embedding-based strategies.
\end{abstract}
...
\begin{enumerate}
    ...
    \item We show that two principled approaches to overcoming the cliff---\textbf{beam search} and \textbf{query-adaptive routing}---fail, demonstrating that the barrier is intrinsic to embedding-based retrieval on dense graphs and pointing toward relation-type-aware methods as the path forward.
\end{enumerate}
...
\paragraph{Implications.} The failure of both approaches confirms that the hop-count cliff is not an artifact of naive single-shot retrieval. Iterative expansion with embedding-guided pruning (beam search) and query-adaptive strategy selection (routing) both fail because they rely on the same underlying signal---embedding similarity---which does not capture relational reasoning. Overcoming the cliff likely requires \textit{relation-type-aware} traversal: methods that select which edge types to follow at each hop, such as Think-on-Graph's LLM-guided traversal~\citep{sun2024tog} or MoR's learned planning stage~\citep{yue2025mor}.
...
We set out to find a phase transition in GraphRAG retrieval performance and found something more fundamental: a \textit{cliff}. On dense knowledge graphs, all retrieval strategies---text-only, path-based, subgraph-based, hybrid, beam search, and adaptive routing---collapse at 2-hop queries. The statistical evidence confirms that structural methods gain relative advantage at higher hop counts, but absolute performance remains too low for practical use.

Through systematic ablation studies, we show that this cliff arises from a fundamental focus-versus-coverage tradeoff inherent to dense graphs. Through beam search and adaptive routing experiments, we further show that the cliff is robust to principled attempts to overcome it within the embedding-based paradigm. The bridge entity problem---intermediate reasoning nodes are not textually similar to the query---renders embedding-guided traversal ineffective for multi-hop reasoning.

The implication is clear: the future of multi-hop GraphRAG on dense knowledge graphs lies not in better embedding-based retrieval strategies but in \textit{relation-type-aware} methods that leverage edge semantics to guide traversal, or in iterative retrieval-generation loops that decompose multi-hop reasoning into manageable single-hop steps.
