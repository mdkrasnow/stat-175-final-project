...
We now develop a theoretical account of the hop-count cliff, drawing on three independent frameworks from spectral graph theory, embedding capacity theory, and the over-squashing literature. All three predict a density-dependent critical hop count $h^*$ beyond which embedding-based retrieval must fail, and their quantitative predictions align with our empirical observations.

\subsection{Neighborhood Explosion and Embedding Capacity}

At hop $h$ from a query entity on a graph with mean degree $\bar{d}$, the expected number of candidate nodes in the $h$-hop neighborhood grows as $|\mathcal{B}_h| \approx \bar{d}^h$~\citep{vanderhofstad2017}. A retrieval system must identify the correct answer among these candidates using cosine similarity in a $D$-dimensional embedding space.

The Johnson-Lindenstrauss lemma~\citep{johnson1984} implies that reliably distinguishing $n$ items in $\mathbb{R}^D$ requires $D = \Omega(\varepsilon^{-2} \log n)$. Inverting this, a fixed $D$-dimensional space can distinguish at most $n^* = O(\exp(\varepsilon^2 D))$ items at distortion $\varepsilon$. \citet{weller2025} sharpen this for embedding-based retrieval, proving a ``crucial point'' $n^*(D)$ beyond which top-$k$ retrieval \textit{must} fail regardless of embedding quality. For $D = 384$ (our all-MiniLM-L6-v2 encoder), their polynomial fit gives $n^* \approx 23{,}000$.

\begin{proposition}[Density-dependent critical hop count]
The critical hop count $h^*$ at which the candidate set exceeds the embedding's discrimination capacity satisfies
\begin{equation}
h^* \approx \frac{\log n^*}{\log \bar{d}}.
\label{eq:hstar}
\end{equation}
\end{proposition}

\begin{proof}
The cliff occurs when $|\mathcal{B}_{h^*}| \approx \bar{d}^{h^*} \geq n^*$. Taking logarithms yields $h^* \log \bar{d} \geq \log n^*$, giving $h^* \approx \log n^* / \log \bar{d}$.
\end{proof}

For our two datasets with $n^* \approx 23{,}000$:
\begin{itemize}
    \item \textbf{PrimeKG} ($\bar{d} = 62.6$): $h^* \approx \log(23000) / \log(62.6) \approx 10.0 / 4.1 \approx 2.4$
    \item \textbf{Amazon} ($\bar{d} = 9.1$): $h^* \approx \log(23000) / \log(9.1) \approx 10.0 / 2.2 \approx 4.5$
\end{itemize}

These predictions match our observations: PrimeKG's cliff occurs between hops 1 and 2 ($h^* \approx 2.4$ correctly predicts the boundary), while Amazon's degradation is gradual through hop 4--5 ($h^* \approx 4.5$). The formula also explains the \textit{sharpness} of the cliff: because $\bar{d}^h$ is exponential in $h$, the neighborhood transitions from ``well within capacity'' to ``far exceeding capacity'' within a single hop increment.

\subsection{Signal Diffusion and the Spectral Gap}

We can also derive the cliff from spectral graph theory. Consider the query entity's identity as a signal on the graph. Under diffusion dynamics, a signal $f(t)$ evolves as $f(t) = \exp(-tL)f(0)$, where $L$ is the graph Laplacian~\citep{spielman2025}. The $i$-th frequency component decays at rate $\exp(-t\lambda_i)$, where $\lambda_i$ are the Laplacian eigenvalues. The signal's discriminative power at hop $h$ is governed by the algebraic connectivity $\lambda_2$ (the smallest nonzero eigenvalue):
\begin{equation}
\text{Signal retention at hop } h \approx \exp\!\left(-h \cdot \frac{\lambda_2}{\bar{d}}\right).
\label{eq:signal}
\end{equation}

The Alon-Boppana bound gives $\lambda_2 \geq \bar{d} - 2\sqrt{\bar{d} - 1}$ for $\bar{d}$-regular graphs~\citep{hoory2006}. Dense graphs have larger $\lambda_2$, meaning faster signal decay---the query entity's ``fingerprint'' washes out in fewer hops.

Substituting approximate values:
\begin{itemize}
    \item \textbf{PrimeKG}: $\lambda_2 / \bar{d} \approx 0.75$, so at $h = 2$: retention $\approx e^{-1.5} \approx 0.22$ (78\% signal loss).
    \item \textbf{Amazon}: $\lambda_2 / \bar{d} \approx 0.36$, so at $h = 2$: retention $\approx e^{-0.72} \approx 0.49$ (51\% signal loss).
\end{itemize}

Our empirical performance retention (averaged across strategies, normalized to 1-hop) is 30\% at 2-hop on PrimeKG and 60\% on Amazon---consistent with these spectral predictions. The discrepancy between theoretical signal retention and empirical retrieval performance is expected, since retrieval requires not just detecting the signal but ranking it above $\bar{d}^h - 1$ distractors.

\subsection{Connection to Over-Squashing}

The over-squashing phenomenon in graph neural networks~\citep{alon2021,topping2022,digiovanni2023} provides a third, independent formalization. Although our retrievers do not use GNNs, the same mathematical bottleneck applies: information about a node $h$ hops away must be discriminated using a fixed-dimensional representation.

\citet{digiovanni2023} prove that for a tree with branching factor $b$, the sensitivity of a node's representation to a node $h$ hops away decays as:
\begin{equation}
\left|\frac{\partial \mathbf{h}_v^{(h)}}{\partial \mathbf{x}_u}\right| \leq C \cdot b^{-h/2}.
\label{eq:oversquash}
\end{equation}

Applying this to our setting with $b \approx \bar{d}$:
\begin{itemize}
    \item \textbf{PrimeKG} at $h = 2$: sensitivity $\propto 62.6^{-1} \approx 0.016$
    \item \textbf{Amazon} at $h = 2$: sensitivity $\propto 9.1^{-1} \approx 0.110$
\end{itemize}

The $\sim$7$\times$ sensitivity ratio is consistent with our observed cliff severity ratio. \citet{topping2022} further show that alleviating over-squashing (by adding edges to increase the spectral gap) necessarily \textit{worsens} over-smoothing---a theoretical manifestation of the focus-vs-coverage tradeoff we observe empirically.

\subsection{Summary of Theoretical Predictions}

Table~\ref{tab:theory} summarizes the convergence of these three frameworks.

\begin{table}[h]
\centering
\caption{Theoretical predictions vs.\ empirical observations. All three frameworks predict the cliff location and density dependence.}
\label{tab:theory}
\begin{tabular}{l cc cc}
\toprule
& \multicolumn{2}{c}{PrimeKG ($\bar{d} = 62.6$)} & \multicolumn{2}{c}{Amazon ($\bar{d} = 9.1$)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Framework & Predicted & Observed & Predicted & Observed \\
\midrule
Critical hop $h^*$ (Eq.~\ref{eq:hstar}) & 2.4 & $\sim$2 & 4.5 & $\sim$4--5 \\
Signal retention at $h{=}2$ (Eq.~\ref{eq:signal}) & 22\% & 30\% & 49\% & 60\% \\
Sensitivity at $h{=}2$ (Eq.~\ref{eq:oversquash}) & 0.016 & --- & 0.110 & --- \\
\bottomrule
\end{tabular}
\end{table}

The unifying insight across all three frameworks is that the cliff emerges from the interaction between \textit{exponential neighborhood growth} ($\bar{d}^h$) and \textit{fixed embedding capacity} ($D$). Dense graphs hit this capacity wall at lower $h$ because $\bar{d}^h$ grows faster. The cliff is sharp rather than gradual because the transition from ``within capacity'' to ``exceeding capacity'' happens within a single hop increment when the base of the exponential is large.
